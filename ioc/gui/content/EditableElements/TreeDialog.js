define([
    ...
    var DojoWioccDialog = declare("ioc.gui.CustomDialog", [Dialog, TemplatedMixin, WidgetsInTemplateMixin, EventObservable, EventObserver], {
        ...
        JSONToData: function(data) {
            ...
            console.log("#### data build START ####")

            let counter = {
                count: 0
            };

            let aux = [this.dataToNode(data, 'root', counter)];

            console.log("*** DATA TO NODE RESULT ***");

            console.log(aux);
            console.log("#### data build END ####");

            return aux;
        },

        // CASOS:
        // el node que arriba és un array:
        //      cada element ha de tenir un id
        //
        // el node que arriba és un objecte
        //      s'ha d'afegir un id
        dataToNode: function (data, name, counter) {
            ...
            if (Array.isArray(data)) {
                ...
                for (let i=0; i<data.length; i++) {
                    // console.log("Array: data to node", i, data[i]);

                    // Hi han dos casos diferents
                    //      - quan un array es troba dins d'un array cal encapsular-lo
                    //      - els elements de l'array no són arrays
                    let value;
                    ...
                    if (Array.isArray(data[i])) {
                        value = {
                            ...
                            name : i,
                            type: 'array',
                            children: this.dataToNode(clonedData[i], i, counter)
                        }
                    } else {
                        value = this.dataToNode(clonedData[i], i, counter);
                    }
                ...
                for (let i in keys) {
                    ...
                    if (Array.isArray(value)) {
                        value = {
                            ...
                            name: key,
                            type: 'array',
                            children: this.dataToNode(value, key, counter)
                        }
                    } else {
                        value = this.dataToNode(value, key, counter);
                    }
        ...
        DataToJSON: function(data) {
          console.error("TODO: fer la conversió adequada per desar")
        },
